...
aec_module.set_stream_format(16000, 1)  # 16kHz, mono
...
def apply_aec(mic_audio_bytes, ref_audio_bytes):
    try:
        mic_np = np.frombuffer(mic_audio_bytes, dtype=np.int16)

        if ref_audio_bytes and isinstance(ref_audio_bytes, (bytes, bytearray)):
            ref_np = np.frombuffer(ref_audio_bytes, dtype=np.int16)
            aec_module.process_reverse_stream(ref_np)
        else:
            ref_np = np.zeros_like(mic_np)

        print("[AEC DEBUG] Ref audio range:", np.min(ref_np), np.max(ref_np), "| Mean:", np.mean(ref_np))
        print("[AEC DEBUG] Mic audio range before AEC:", np.min(mic_np), np.max(mic_np), "| Mean:", np.mean(mic_np))

        processed = aec_module.process_stream(mic_np)
        processed_np = np.frombuffer(processed, dtype=np.int16)
        print("[AEC DEBUG] Output audio range:", np.min(processed_np), np.max(processed_np), "| Mean:", np.mean(processed_np))

        return processed

    except Exception as e:
        print(f"[AEC] Error in apply_aec: {e}")
        return mic_audio_bytes
...
# ========== GLOBAL STATE ==========
aec_module = AudioProcessingModule()
aec_module.set_stream_format(16000, 1)  # sample rate, channels
aec_module.set_ns_level(1)              # Noise suppression level (0-3)
aec_module.set_vad_level(1)             # VAD sensitivity level (0-3)
...
def background_vad_listener():
    ...
    min_bargein_delay = 1.8  # ⏱ allow TTS to fully play into mic before enabling barge-in
    ...
    try:
        with sd.InputStream(device=MIC_DEVICE_INDEX, samplerate=MIC_SAMPLE_RATE, channels=1, dtype='int16', blocksize=blocksize) as stream:
            while buddy_talking.is_set():
                ...
                mic_audio = frame.flatten().tobytes()  # raw mic input
                ref_copy = ref_audio_buffer.copy()     # recent Buddy voice audio

                # ✅ Apply echo cancellation
                aec_fixed = apply_aec(mic_audio, ref_copy)

                # ✅ Downsample to 16kHz for VAD
                audio_16k = downsample(np.frombuffer(aec_fixed, dtype=np.int16), MIC_SAMPLE_RATE, 16000)

                # ✅ Avoid premature interruption due to reference echo
...
def tts_worker():
    ...
    while True:
        try:
            ...
            if not text.strip():
                print("[Buddy][TTS_WORKER] Empty text. Skipping.")
                tts_queue.task_done()
                continue

            print(f"[Buddy][TTS_WORKER] About to TTS: '{text}', lang={lang}, style={style}")

            # ✅ Locking to avoid overlapping playback
            with playback_lock:
...
def audio_playback_worker():
    print("[Buddy][Playback] Audio playback thread started")
    while True:
        try:
            ...
            # Convert AudioSegment to NumPy for sounddevice
            audio_np = np.array(audio.get_array_of_samples()).astype(np.int16)
            sd.play(audio_np, samplerate=audio.frame_rate)
            sd.wait()
    ...
    with sd.InputStream(device=MIC_DEVICE_INDEX, samplerate=MIC_SAMPLE_RATE, channels=1, blocksize=blocksize, dtype='int16') as stream:
        ...
        while True:
            ...
            for i in range(0, len(mic_16k), 160):
                ...
                # Inject echo reference before processing mic
                with ref_audio_lock:
                    ap.process_reverse_stream(ref_audio_buffer.tobytes())
...
def ask_llama3_openai_streaming(messages, model="llama3", max_tokens=60, temperature=0.5, lang="en", style=None):
    ...
    try:
        with requests.post(url, json=payload, stream=True, timeout=120) as response:
            ...
            # ✅ Final fallback
            if not spoken_anything and full_response.strip():
                print(f"[Buddy] ==>> Buddy mówi: {full_response.strip()}")
                while buddy_talking.is_set():
                    time.sleep(0.05)
                with tts_lock:
                    speak_async(full_response.strip(), lang=lang, style=style)
...
def handle_user_interaction(speaker, history, conversation_mode="auto"):
    ...
    # ✅ NEW: Handle full-duplex barge-in
    if full_duplex_interrupt_flag.is_set():
        if DEBUG:
            print("[Buddy] Interrupted during speaking — resetting TTS and handling new input.")
        full_duplex_interrupt_flag.clear()
        return True
    ...
    if vad_triggered.is_set():
        if DEBUG:
            print("[Buddy] Barge-in: live TTS stopped, moving to new question.")
        vad_triggered.clear()
...
def generate_and_play_kokoro(text, lang=None):
    ...
    try:
        ...
        print(f"[DEBUG][TTS] About to call kokoro.create with: text={text!r}, voice={voice!r}, lang={kokoro_lang!r}")
        ...
        if len(samples) < 160:
            print(f"[Buddy][TTS] Warning: Skipping TTS — audio too short (len={len(samples)})")
            return

        print(f"[DEBUG][TTS] kokoro.create returned: type(samples)={type(samples)}, len={len(samples)}, sample_rate={sample_rate}")
        print(f"[DEBUG][TTS] First 10 samples: {samples[:10]}")
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as f:
            ...
            print(f"[DEBUG] Audio segment frame rate: {audio.frame_rate}, channels: {audio.channels}")
            debug_path = os.path.abspath("buddy_tts_debug.wav")
            audio.export(debug_path, format="wav")
            print(f"[Buddy][TTS] Exported debug wav to: {debug_path}")

        # 🔊 Inject this as echo reference
        set_ref_audio(samples.tobytes())  # Safe, because it's already PCM 16

        global tts_start_time
        tts_start_time = time.time()  # 🕒 delay VAD trigger

        # ▶️ Wait until previous audio finishes before queuing
        with playback_lock:
            while buddy_talking.is_set():
                time.sleep(0.1)
            buddy_talking.set()  # ✅ CRITICAL LINE: tell playback Buddy is speaking
