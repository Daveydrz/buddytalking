...
def speak_async(text, lang=DEFAULT_LANG, style=None):
    print(f"[Buddy][SPEAK_ASYNC] Queuing text for TTS: {text}")
    if not text.strip():
        return
...
def ask_llama3_openai_streaming(messages, model="llama3", max_tokens=60, temperature=0.5, lang="en", style=None):
    ...
    try:
        with requests.post(url, json=payload, stream=True, timeout=120) as response:
            ...
            # Speak any leftover buffer (chunked)
            if buffer.strip():
            ...
            # If nothing at all was spoken, speak the full response as a fallback
